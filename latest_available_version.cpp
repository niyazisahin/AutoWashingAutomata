...
void loop()
{
  ...
  if(digitalRead(7) && !bitis_flag){
    ...
    Serial.println(" TL");
    Serial.println(kasa[2]);
    Serial.println(hizmetler[0].hizmetAd);
  }
  ...
  if(bitis_flag){
    ...
    for(int i = 0; i<4; i++){
      ...
      if(!bitis_flag2 && bekle%1000 == 0){
          Serial.print(i);
        	Serial.print(". ");
          Serial.print(hizmetler[i].hizmetAd);
        	Serial.print(" : ");
      ...
      if(kasaParasi >= hizmetParasi)
      {
        ...
        for(struct hizmet &x : hizmetler){
          ...
          if(x.hizmetID == 1){
            ...
            if(kopukMiktar > x.kalanHizmetAdet){
            	kopukMiktar = x.kalanHizmetAdet;
                Serial.println("Kopukleme Icin Alinan Adet Fazladir.\nVerilebilecek Max Adet Veriliyor... ");
                Serial.print(kopukMiktar);
                Serial.println(" Adet Veriliyor...");
            ...
            if(yikamaMiktar > x.kalanHizmetAdet){
            	yikamaMiktar = x.kalanHizmetAdet;
                Serial.println("Yıkama Icin Alinan Adet Fazladir.\nVerilebilecek Max Adet Veriliyor... ");
                Serial.print(kopukMiktar);
                Serial.println(" Adet Veriliyor...");
            }
            ...
            if(kurulamaMiktar > x.kalanHizmetAdet){
            	kurulamaMiktar = x.kalanHizmetAdet;
              	Serial.println("Kurulama Icin Alinan Adet Fazladir.\nVerilebilecek Max Adet Veriliyor... ");
                Serial.print(kopukMiktar);
                Serial.println(" Adet Veriliyor...");
            }
            ...
            if(cilalamaMiktar > x.kalanHizmetAdet){
            	cilalamaMiktar = x.kalanHizmetAdet;
                Serial.println("Cilalama Icin Alinan Adet Fazladir.\nVerilebilecek Max Adet Veriliyor... ");
                Serial.print(kopukMiktar);
                Serial.println(" Adet Veriliyor...");
            }
        ...
        while(paraUstu > 0){
          ...
          else if(paraUstu >= 50 && kasa[3] > 0){
            Serial.println("50 tl bankot veriliyor");
            paraUstu -= 50;
            kasa[3]--;
          }
          
          else if(paraUstu >= 20 && kasa[2] > 0){
            Serial.println("20 tl bankot veriliyor");
            paraUstu -= 20;
            kasa[2]--;
          }
          
          else if(paraUstu >= 10 && kasa[1] > 0){
            Serial.println("10 tl bankot veriliyor");
            paraUstu -= 10;
            kasa[1]--;
          }
          
          else if(paraUstu >= 5 && kasa[0] > 0){
            Serial.println("5 tl bankot veriliyor");
            paraUstu -= 5;
			kasa[0]--;
          }else
          {
            Serial.println("Kasada Yeterli Sayida Banknot Yoktur.");
            for(struct hizmet &x : hizmetler)
            {
              if(x.hizmetID == 1)
                 x.kalanHizmetAdet += kopukMiktar;
